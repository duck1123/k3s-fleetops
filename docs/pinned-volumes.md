...
- **`sync-options: Replace=true,Force=true`** on both the PVC and PV (already set by `mkPinnedVolume`) is required once a volume is pinned -- without it, ArgoCD's normal apply semantics choke on `volumeName`/`storageClassName` being effectively-immutable fields if the object already exists with different values.
- **Switching a volume from unpinned to pinned changes its PVC's `storageClassName`** (from the app's normal one, e.g. `"longhorn"`, to `""`) and its `volumeName` (from the dynamically-provisioned PV to the new pinned one) -- both immutable on an existing *bound* PVC. `Replace=true` alone is not enough here: it only gets ArgoCD to attempt a plain `kubectl replace` (a PUT), which Kubernetes rejects for a bound PVC's immutable fields with "spec is immutable after creation" -- and ArgoCD does not fall back to a delete+recreate on that failure by itself, it just retries the same failing replace on every sync until it exhausts its retry budget. `Force=true` is what makes ArgoCD actually delete-and-recreate the object when a replace hits that wall. This is a real, if brief, unmount/remount of anything actively using the volume -- fine for most of these apps, but worth timing deliberately rather than doing it blind on something with active writes in flight. (Confirmed the hard way pinning `ditto-relay`'s OpenSearch volume before `Force=true` was added: ArgoCD retried the failing replace 5 times and gave up, and the fix at the time was a manual `kubectl scale ... --replicas=0` + `kubectl delete pvc` to let it recreate cleanly.)
