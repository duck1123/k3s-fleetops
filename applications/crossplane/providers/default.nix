...
let
  ...
  providerDirs = builtins.filter (
    name: (builtins.getAttr name (builtins.readDir providersDir)) == "directory"
  ) (builtins.attrNames (builtins.readDir providersDir));

  # Import each provider's `default.nix`
  providerModules = map (dir: import (providersDir + "/${dir}/default.nix")) providerDirs;
in
{
