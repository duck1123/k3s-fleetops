...
let
  ...
  mkTimeseriesPanel = { id, title, expr ? null, legendFormat ? null, unit ? "short", gridPos, thresholds ? null, overrides ? [ ], min ? null, max ? null, targets ? null }:
    let
      defaultTargets = if expr != null && legendFormat != null then [
        {
    ...
    in
    {
      ...
      type = "timeseries";
      fieldConfig = {
        defaults = timeseriesFieldConfig // {
          inherit unit;
  ...
  mkStatPanel = { id, title, expr, unit ? "short", gridPos, thresholds ? null }:
    {
      inherit id title;
      datasource = "Prometheus";
      type = "stat";
  ...
  mkDashboard = { title, uid, tags ? [ ], panels, refresh ? "30s", timeFrom ? "now-6h", timeTo ? "now" }:
    {
      annotations = {
        list = [
          {
